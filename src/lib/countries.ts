...
export function countryName(code: string): string | null {
  const match = BY_CODE.get(code.trim().toUpperCase());
...
export function findCountry(query: string): Country | null {
  ...
  const byCode = BY_CODE.get(upper);
  if (byCode) {
    return byCode;
  }
  const lower = trimmed.toLowerCase();
  const exactName = COUNTRIES.find(
    (country) => country.name.toLowerCase() === lower,
  );
  if (exactName) {
    return exactName;
  }
  const namePrefix = COUNTRIES.find((country) =>
    country.name.toLowerCase().startsWith(lower),
  );
  if (namePrefix) {
    return namePrefix;
  }
  const nameContains = COUNTRIES.find((country) =>
